...
기본 순서는 **계획 → 화면 목업 → 계획서에 반영 → 개발 현황 → 만들기** 입니다. 목업을 반영하면 그대로 이어서 만들기가 시작되고, 목업을 건너뛰고 `목업 없이 만들기` 를 눌러도 마찬가지입니다 — 어느 쪽이든 `docs/PROGRESS.json` 이 없으면 **그것부터 만든 다음** 코드를 씁니다. 현황 파일이 만들어지면 개발 현황 창이 스스로 열립니다.
...
**1. `대화에서 모으기`** — 대화창에서 시킨 요청을 모아 `docs/CHANGES.md` 에 적고, **코드에서 원인을 확인해 지시 한 문장으로 다듬습니다.** 평소에 따로 적어 둘 것이 없습니다.
...
> 친 말 그대로 늘어놓으면 목록이 대화 원문 더미가 됩니다. 그래서 모은 직후에 **친 말(증상)과 코드에서 확인한 원인을 합쳐** 지시 한 문장으로 줄입니다.
>
> ```markdown
> - [ ] `09-06 03:18` 타임라인 슬라이더는 끄는 동안 화면이 한 프레임 안에 따라오도록,
>       이동마다 전체를 다시 그리는 seek 을 requestAnimationFrame 으로 묶어서 처리해주세요.
> ```
>
> **어떤 부분을 · 어떻게 되도록 · 어떤 식으로** 셋이 한 문장에 들어갑니다. `재생 속도 개선` 같은 이름표로 줄이면 반영할 때 어디를 만져야 하는지를 처음부터 다시 조사하게 됩니다.
>
> 원인을 확인하지 못하면 `어떤 식으로` 는 비워 둡니다 — **지어내지 않습니다.** 비어 있으면 반영할 때 다시 봅니다.
>
> 친 말은 이 문장으로 **덮어써집니다.** 그래서 뜻을 바꾸지 않고, 확실하지 않은 대목은 쓴 표현을 그대로 살립니다. 원문이 필요하면 `docs/CHANGES.md` 를 `git diff` 로 보거나 대화 기록에서 다시 모을 수 있습니다.
...
| 문장의 자리 | 가는 곳 |
|:--|:--|
| `어떻게 되도록` | `BUILD-PLAN.md` — `타임라인 슬라이더를 끄는 동안 화면이 한 프레임 안에 따라온다` |
| `어떤 식으로` | `BUILD-LOG.md` — `이동마다 draw 전체가 돌고 있었다. seek 을 rAF 로 묶어…` |

**계획서에는 방법이 아니라 만족해야 할 상태를 적습니다.** 방법이 바뀌어도 사양은 남습니다.

반영한 항목은 `반영함` 으로 내려가면서 **실제로 무엇을 고쳤는지가 `결과:` 로 따라붙습니다.** 요청만 남으면 그건 시킨 것의 목록일 뿐, 변경 기록이 아닙니다.
